...
variants = [
    'кот', 'шеф', 'мозг', 'лес', 'фолк', 'код', 'рот', 'мёд', 'лук', 'лес', 'год', 'час', 'друг', 'муж', 'айфон',
    'стол', 'нос', 'сыр', 'хлеб', 'мир', 'свет', 'рок', 'дед', 'дом', 'сон', 'глаз'
]
...
for i in range(num_proverbs):
    if not proverbs or not variants:
        print("Недостаточно данных для генерации пословиц.")
        break
    
    proverb = proverbs.pop()
    variant = variants.pop()
    
    new_proverb = proverb.replace("Ум", variant.capitalize())
