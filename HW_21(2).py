...
while True:
    ...
    if last_city and not (user_city in cities_set and user_city.startswith(last_letter)):
...
while True:
    if last_city:
        last_letter = last_city[last_city.rfind(last_city[-1])].upper() if last_city else ''
        print(f"Ваш ход. Назовите город на букву '{last_letter}'.")
    else:
        print("Ваш ход. Назовите любой город.")
...
# Если нет города на последнюю букву, пробуем предпоследнюю букву
    if not computer_city:
        last_letter = last_city[last_city.rfind(last_city[-2])].upper() if len(last_city) > 1 else ''
        for city in cities_set:
            if city.startswith(last_letter):
                computer_city = city
                break
